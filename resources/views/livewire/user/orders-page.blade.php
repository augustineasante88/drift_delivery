...
<div class="w-full grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
...
<div wire:click='openOrderDetailsModal({{$order->id}})' class="hover:cursor-pointer relative overflow-hidden rounded-md bg-white shadow-lg ">
          ...
          <div class="m-2.5 flex justify-between">
            <div class="text-gray-400 font-medium">{{$order->created_at}}</div>
...
{{-- <div wire:click='openOrderDetailsModal("{{$order->id}}")' class="relative cursor-pointer h-40 w-full space-y-3 overflow-hidden rounded-2xl bg-blue-100 bg-gradient-to-t from-blue-50 to-blue-300 shadow-md">
    <div class="flex justify-between p-4">
      <p class="text-base font-medium text-gray-600">#order: {{$order->id}}</p>
      @if($order->status == 4)
      <span class="rounded-full font-semibold border border-yellow-300 bg-yellow-100 px-2 text-yellow-700">pending</span>
      @elseif($order->status == 2)
      <span class="rounded-full font-semibold border border-blue-300 bg-blue-100 px-2 text-blue-700">Picked up</span>
      @elseif($order->status == 3)
      <span class="rounded-full font-semibold border border-green-300 bg-green-100 px-2 text-green-700">Delivered</span>
      @elseif($order->status == 1)
      <span class="rounded-full border font-semibold border-red-300 bg-red-100 px-2 text-red-700">cancelled</span>
      @endif
    </div>
    <div class="flex items-baseline justify-between px-4">
      <p class="text-4xl">{{$order->total}}</p>
      <p class="text-base font-medium text-gray-600">{{$order->created_at}}</p>
    </div>
    <div class="absolute bottom-0 flex w-full justify-center items-center bg-white py-1 space-x-2">
      <p wire:loading.remove wire:target='openOrderDetailsModal("{{$order->id}}")' class='text-lg font-semibold'>view details</p>
...
<p class='text-xl font-semibold'> You have no pending order</p>
</div>
@endforelse
    ...
    <div class="mt-8 mb-2">
        <div class="flow-root">
          ...
          <ul role="list" class="-my-6 divide-y divide-gray-200">
              ...
              <li class="flex py-6" wire:key="{{$loop->index}}">
      ...
      <div class='mt-8 border-t'>
        <h3 class='text-base font-normal flex'>Your delivery guy is <span class='text-blue-500 ml-1'>{{$bikerName}}</span></h3>
      ...
      @else
      <div class='mt-8 border-t'>
        <h3 class='text-xs text-green-500 font-normal flex'>Your order has been recieved and is pending to be assigned to a delivery guy. Please be patient...</h3>
      </div>
      @endif
        ...
        <div class="flex justify-between gap-x-4">
          ...
          @endif
            ...
            <div class="flex">
                ...
                @if($orderStatus == 4)
                <x-button outline warning label="Your order is pending" />
                @elseif($orderStatus == 1)
                <x-button outline negative label="Your order is Cancelled" />
                @elseif($orderStatus == 3)
                <x-button outline purple label="Your package has arrived" />
                @elseif($orderStatus == 5)
                <x-button outline emerald label="Delivered" />
                @elseif($orderStatus == 2)
                <x-button outline info label="Your order is on the way" />
                @endif
            </div>
        </div>
    </x-slot>
